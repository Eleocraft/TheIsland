    ...
    public string InteractionInfo
    {
        get
        {
            if (state == BrokenRaftState.Inactive)
                return $"press {GlobalData.controls.PlayerFP.Interaction.bindings[0].ToDisplayString()} to Inspekt the Raft";
            else if (state == BrokenRaftState.Repairing)
            {
                ...
                return $"press {GlobalData.controls.PlayerFP.Interaction.bindings[0].ToDisplayString()} to Repair the Raft";;
        }
    }
